...
void PhotonMap::tracePhotons(const std::shared_ptr<Scene> scene, const uint32_t numPhotons)
{
...
bool PhotonMap::tracePhoton(const std::shared_ptr<Scene> scene, Photon& photon, uint32_t depth,
                            float eta_i)
{
    if (depth > 5)
    {
    ...
    // intersect scene
    std::vector<std::shared_ptr<Prim>> prims = scene->getPrims();
    ...
    else
    {
        ...
        BsdfSample sample =
            bsdf.sample(rInter.hitPoint, -photonRay.d, n, eta_i, rInter.mat->getIor());
